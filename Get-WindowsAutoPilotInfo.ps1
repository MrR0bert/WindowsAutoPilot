...
Process
{
	foreach ($comp in $Name)
	{
		...
		# Create the base object, forcing the correct sort order
		$Props = [Ordered]@{
		...
		if ($Partner)
		{
			# Add two properties to the props list for Partner format
			$Props.Add( "Manufacturer name", $make )
			$Props.Add( "Device model", $model )
		}
		elseif ($GroupTag -ne "")
		{
			$Props.Add( "Group Tag", $GroupTag )
		}
